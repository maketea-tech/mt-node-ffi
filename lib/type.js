...
function Type(type) {
  ...
  // if "rtn" isn't set (ffi_type was not set) then we check for "mt-node-ref-array" type
  if (!rtn && type.type) {
    // got a "mt-node-ref-array" type
    // passing arrays to C functions are always by reference, so we use "pointer"
    rtn = bindings.FFI_TYPES.pointer;
  }

  if (!rtn && type.fields) {
    // got a "mt-node-ref-struct" type
    // need to create the `ffi_type` instance manually
    debug('creating an `ffi_type` for given "mt-node-ref-struct" type');
    ...
    for (i = 0; i < numFields; i++) {
      ...
      if (field.type.fixedLength > 0) {
        // a fixed-length "mt-node-ref-array" type
